...
- **Data.** Give every daemon a fresh directory made with `mktemp -d` under
  ...
  - CLI: `--data-dir "$dir/data" --download-dir "$dir/downloads"`.
  - App: `--dart-define=MYCONNECT_DATA_DIR=$dir/data` and
    `--dart-define=MYCONNECT_DOWNLOAD_DIR=$dir/downloads`. Without the
    download dir, received files land in the owner's `~/Downloads`.
...
- **Display and D-Bus.** Run the app under
  ...
  checked is free. The Linux app is a unique `GApplication` per D-Bus
...
worktree's `ui/` before anything else.
