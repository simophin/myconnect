...
| `ui` | `src/ui/{mod,plugin,sync,demo}.rs`, `src/ui/pages/*.rs` | The native desktop UI in iced, behind the `gui` feature ([`adr/0001`](adr/0001-native-ui-in-iced.md), [`PLAN_ICED_UI.md`](PLAN_ICED_UI.md)). It runs in the daemon's process and reads the core directly: `sync` subscribes to the event bus, then takes a snapshot, and takes a fresh one after a lag. It never names a feature: each feature's UI half will live in `src/plugins/<name>/ui.rs`, plug in through `ui::plugin`, and be listed by `plugins::builtin_with_ui()`. `demo` fills the core with made-up devices for `--demo`. |
