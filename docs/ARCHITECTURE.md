...
| `config` | `src/config/{mod,identity,settings,token}.rs` | Local device identity (UUID + self-signed cert), the optional API bearer token (never persisted), and `settings.json` (user settings, written atomically). |
| `store` | `src/store/{mod,config,devices}.rs` | The daemon's data in one SQLite database, `ferry.db` in the data directory ([`adr/0002`](adr/0002-store-the-daemons-data-in-sqlite.md), [`PLAN_STORE.md`](PLAN_STORE.md)). `config`: typed, watchable values, each named by a `ConfigKey<T, S>` its owner declares, global or per device (`PerDevice`, reached with `.of(id)`). `devices`: paired devices' pinned certificates, with the name, type and capabilities each last reported over an authenticated connection; removing one removes its per-device values. Tests use `Store::open_in_memory()`. |
