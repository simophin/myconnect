...
| `ui` | `src/ui/{mod,plugin,route,sync,demo}.rs`, `src/ui/pages/*.rs` | The native desktop UI in iced, behind the `gui` feature ([`adr/0001`](adr/0001-native-ui-in-iced.md), [`PLAN_ICED_UI.md`](PLAN_ICED_UI.md)). It runs in the daemon's process and reads the core directly: `sync` subscribes to the event bus, then takes a snapshot, and takes a fresh one after a lag. It never names a feature: each feature's UI half lives in `src/plugins/<name>/ui.rs` (battery's so far), implements `ui::plugin::UiPlugin` with its own message type, and is listed by `plugins::builtin_with_ui()`. The shell stores them erased, routes their messages back by plugin id, fills its slots from them (the device card's status chips), and does what they ask through `ShellRequest` (toast, navigate). `demo` fills the core with made-up devices for `--demo`; what they report comes from each plugin's `demo_packets`. |
